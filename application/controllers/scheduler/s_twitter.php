...
class S_Twitter_Controller extends Controller {
	...
	*/
	private function add_hash_tweets($data)
	{
		...
		// Skip the rest if our $data = false
		...
		foreach($tweet_results->statuses as $tweet)
		{
			...
			// DB Expression used to do case insensitive comparison on twitter handle
			$expr = new Database_Expression('LCASE(`service_account`) = :name', array(':name' => utf8::strtolower($tweet->user->screen_name)));
					...
					->find_all()) == 0)
			{
				...
				if ($reporter_weight > 0 AND $reporter_location)
				{
					...
					// Save reporter categories as incident categories
