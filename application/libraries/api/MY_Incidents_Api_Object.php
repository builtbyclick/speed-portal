...
class Incidents_Api_Object extends Api_Object_Core {
	...
	public function _get_incidents($where = array())
	{
		...
		// Make original params still work
		...
		// Map order field
		$order_map = array(
			'i.incident_title' => 'title',
			'i.incident_date' => 'date',
			'i.id' => 'id'
		);
		$_GET['order'] = $order_map[$this->order_field];
		// If sorting 0/1
		if (isset($_GET['sort']) AND $_GET['sort'] != 'ASC' AND $_GET['sort'] != 'DESC')
		{
			$_GET['sort'] = $this->sort;
		}

		//$items = Incident_Model::get_incidents($where, $this->list_limit, $this->order_field, $this->sort);
		$items = reports::fetch_incidents(TRUE, $this->list_limit);
		...
		$incidents_ids = array();
		$sharing_incident_ids = array();
		$custom_field_items = array();
		
		foreach ($items as $item)
		{		
			if(! isset($item->source) OR $item->source == 'main')
			{
        $incident_ids[] = $item->incident_id;       
			}
			else
			{
				$sharing_incident_ids[] = $item->incident_id;
			}
		...
		if (count($incident_ids) > 0)
		{
		  $incident_categories = ORM::factory('category')
  			...
  			->where('category.category_visible', '1')
  			->where('(c_parent.category_visible = 1 OR c_parent.id IS NULL)')
  			->find_all();
		}
		...
		{
		  $sharing_incident_categories = array();
		  
		  if (count($sharing_incident_ids) > 0)
		  {
		    $sharing_incident_categories = ORM::factory('category')
  				->select('category.*, sharing_incident_category.sharing_incident_id AS incident_id')
  				->join('category AS '.$table_prefix.'c_parent', 'category.parent_id', 'c_parent.id', 'LEFT')
  				->join('sharing_incident_category','category.id','sharing_incident_category.category_id')
  				->in('sharing_incident_category.sharing_incident_id', $sharing_incident_ids)
  				->where('category.category_visible', '1')
  				->where('(c_parent.category_visible = 1 OR c_parent.id IS NULL)')
  				->find_all();
		  }

			$sharing_category_items = array();

			// Fetch items into array
			foreach ($sharing_incident_categories as $incident_category)
			{
				$sharing_category_items[$incident_category->incident_id][] = $incident_category->as_array();
			}
		}
		...
		foreach ($items as $item)
		{
			...
			$xml->writeElement('udate', strtotime($item->incident_date));
			...
			if (isset($item->source) AND $item->source != 'main' AND isset($sharing_category_items[$item->incident_id]))
			{
				foreach ($sharing_category_items[$item->incident_id] as $category_item)
				{
			...
			elseif (isset($category_items[$item->incident_id]))
			{
			...
			if ($this->response_type == 'json' OR $this->response_type == 'jsonp')
			{
				$json_reports[] = array(
					"incident" => array(
						...
						"incidentudate" => strtotime($item->incident_date),
						...
						"sharingsourceid" => isset($item->source) ? $item->source : 'main',
						"sharingsourceurl" => isset($item->source_url) ? $item->source_url : URL::site()
					),
