...
class blocks_Core {
	 ...
	 *
	 * @param int $limit  max number of blocks to render
	 * @param int $offset number of blocks to skip
	 * @return string block html
	 */	
	public static function render($limit = FALSE, $offset = 0)
	{
		// Get Active Blocks
		$active_blocks = Settings_Model::get_setting('blocks');
		$active_blocks = array_filter(explode("|", $active_blocks));
		
		// Slice array based on limit/offset
		if ($limit)
		{
			$active_blocks = array_slice($active_blocks, $offset, $limit);
		}
		else
		{
			$active_blocks = array_slice($active_blocks, $offset);
		}
