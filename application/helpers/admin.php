...
class admin_Core {
	...
	public static function main_tabs()
	{
		...
		else
		{
			...
			$tabs['reporters'] = Kohana::lang('ui_admin.reporters');
	...
	/**
	 * Generate Reporters Tab Menus
	 * @param int $service_id
	 * @return string $menu
	 */
	public static function reporters_subsubtabs($service_id = FALSE)
	{
		$menu = "";
		
		//FIXIT: Kludge first entry in list because array add isn't working here
		if ($service_id == '0' or $service_id == FALSE)
		{
			$menu .= "<li><a class=\"active\">Show all</a></li>";
		}
		else
		{
		  $menu .= "<li><a href=\"" . url::site() . "admin/reporters/index/0\">Show all</a></li>";
		}
		
		//Add all services listed in the database
		foreach (ORM::factory('service')->find_all() as $service)
		{
			if ($service_id == $service->id)
			{
				$menu .= "<li><a class=\"active\">".$service->service_name."</a></li>";
			}
			else
			{
				$menu .= "<li><a href=\"" . url::site() . "admin/reporters/index/".$service->id."\">".$service->service_name."</a></li>";
			}
		}
		...
		/**
	 * Generate Reporter Sub Tab Menus
	 * @param string $this_sub_page
	 * @return string $menu
	 */
	public static function reporters_subtabs($this_sub_page = "view")
	{
		$menu = "";

		$menu .= ($this_sub_page == "view") ? Kohana::lang('ui_main.view_reporters') : "<a href=\"".url::base()."admin/reporters\">".Kohana::lang('ui_main.view_reporters')."</a>";

		$menu .= ($this_sub_page == "edit") ? Kohana::lang('ui_main.create_reporter') : "<a href=\"".url::base()."admin/reporters/edit\">".Kohana::lang('ui_main.create_reporter')."</a>";

		$menu .= ($this_sub_page == "download") ? Kohana::lang('ui_main.download_reporters') : "<a href=\"".url::base()."admin/reporters/download\">".Kohana::lang('ui_main.download_reporters')."</a>";

		$menu .= ($this_sub_page == "upload") ? Kohana::lang('ui_main.upload_reporters') : "<a href=\"".url::base()."admin/reporters/upload\">".Kohana::lang('ui_main.upload_reporters')."</a>";
