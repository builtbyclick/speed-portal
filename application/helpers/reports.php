...
class reports_Core {
	 ...
	 *
	 */
	public static function save_media($post, $incident, $source = "User")
	{
		...
		$delete_query = ORM::factory('media')
			->where('incident_id',$incident->id)
			->where('media_type <> 1');
		if (class_exists('smap_helper'))
		{
			$delete_query->notin('media_type', smap_helper::smap_data_type_mapping());
		}
		...
		// c. Photos
		// These come in two flavours: photos that have been uploaded by the user via a form,
		// and a list of urls for photos that have been downloaded via the SMAP api
		if ( isset($post->incident_photo) )
		{
			//Create list of filenames to process
			$filenames = array();
			if ( $source == "SMAP" )
			{
				$filenames = $post->incident_photo;
			}
			else
			{
				//incident_photo: photos have already been saved by POST in _FILES[]
			...
			foreach ($filenames as $filename)
			{
				...
				// Okay, now we have these three different files on the server, now check to see
				//   if we should be dropping them on the CDN (Content Delivery Network)
