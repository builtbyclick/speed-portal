...
class download_Core {
	 ...
	 * @param Validation $post Validation object with the download criteria 
	 * @param array $ireporters Reporters to be downloaded
	 * @return string $reporter_csv CSV content
	 */
	public static function download_reporter_csv($post, $reporters)
	{
		// Set up output file
		$fiveMBs = 5 * 1024 * 1024;
		$fp = fopen("php://temp/maxmemory:$fiveMBs", 'r+');
		...
		$csv_headers = array("#","SERVICE ID","SERVICE ACCOUNT","LEVEL ID", "FIRST NAME", "LAST NAME", 
		"LOCATION NAME", "LATITUDE", "LONGITUDE", "EMAIL", "PHONE", "URL", "USER ID", "DATE ADDED");
		fwrite($fp, self::arrayToCsv($csv_headers));

		// Write each reporter's details to file
		foreach ($reporters as $reporter)
		{
			if (in_array($reporter->service_id, $post->channel_include) and
			    in_array($reporter->level_id, $post->level_include) )
			{
				//Convert service id to service name - confuses the client less
				//FIXIT: move this code outside the loop to speed up the download
				$service = ORM::factory('service')
					->where('id', $reporter->service_id)
					->find();

				$csv_line = array();
				$csv_line[] = $reporter->id;
				$csv_line[] = $service->service_name;
				$csv_line[] = $reporter->service_account;
				$csv_line[] = $reporter->level_id;
				$csv_line[] = $reporter->reporter_first;
				$csv_line[] = $reporter->reporter_last;
				$csv_line[] = $reporter->location->location_name;
				$csv_line[] = $reporter->location->latitude;
				$csv_line[] = $reporter->location->longitude;
				$csv_line[] = $reporter->reporter_email;
				$csv_line[] = $reporter->reporter_phone;
				$csv_line[] = $reporter->reporter_ip;
				$csv_line[] = $reporter->user_id;
				$csv_line[] = $reporter->reporter_date;
		...
		/**
	 * Download Reporters in XML format
	 * @param Validation $post Validation object with the download criteria 
	 * @param array $ireporters Reporters to be downloaded
	 * @return string $reporter_xml XML content
	 */
	public static function download_reporter_xml($post, $reporters)
	{
		//FIXIT: make this function work
		...
		// Write Column Titles to file
		// Report elements
		$report_elements = array('id', 'service id', 'service account', 'level id', 'first name', 'last name', 
		'location name', 'latitude', 'longitude', 'email', 'phone', 'url', 'user id', 'dateadd');
		...
		/* Start reporters element */
		$writer->startElement('reporters');

		// Write each reporter's details to file
		$rcount = 0;
		foreach ($reporters as $reporter)
		{
			if (in_array($reporter->service_id, $post->channel_include))
			{
				// Start Individual report
				$writer->startElement('reporter');
								
				// Generate report map
				$reporter_element_map = xml::generate_element_attribute_map($reporter, $reporter_map);

				$csv_line = array();
				$csv_line[] = $reporter->id;
				$csv_line[] = $reporter->service_id;
				$csv_line[] = $reporter->service_account;
				$csv_line[] = $reporter->level_id;
				$csv_line[] = $reporter->reporter_first;
				$csv_line[] = $reporter->reporter_last;
		...
		if ($rcount == 0)
		{
			$writer->text('There are no reporters in this file.');
		}
		
		/* Close reporters Element */	
		$writer->endElement();
