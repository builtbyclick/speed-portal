...
<?php if( empty($smap_embed) AND count($incident_photos) > 0) : ?>
		<?php
...
if (!empty($smap_embed)) {
	foreach ($smap_embed as $embed) {
	?>
		<iframe src="<?php echo html::escape($embed->media_link); ?>" class="main" style="height: 400px;"></iframe>
<?php 
	}
}

if ($smap_embed_type != 'map') : ?>
...
<div id="main" class="report_detail">
	...
	<div class="left-col">
		...
		<div class="<?php if( count($incident_photos) > 0 || count($incident_videos) > 0){ echo "report-media";}?>">
			...
			if( empty($smap_embed) AND count($incident_photos) > 0 )
			{
			...
			if( empty($smap_embed) AND count($incident_videos) > 0 )
			{
