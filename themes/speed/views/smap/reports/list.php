...
?>
<?php
$current_page = is_object($pagination) ? $pagination->current_page : 1;
if ($current_page != 1): ?>
		...
		<div class="rb_list-and-map-box">
			...
			<?php
				foreach ($incidents as $i => $incident)
				{
					...
					$incident_url = url::site('smap/view/'.$incident_id);//Incident_Model::get_url($incident_id);
					//$incident_category = $incident->incident_category;
					// Trim to 150 characters without cutting words
					// XXX: Perhaps declare 150 as constant
					...
					if ($media->count())
					{
						foreach ($media as $photo)
						{
							...
							{ // Get the first thumb
								if ($pagination->current_page == 1 AND $i == 0) { $incident_thumb = url::convert_uploaded_to_abs($photo->media_link); }
								else {
				...
				<div id="incident_<?php echo $incident_id ?>" class="rb_report <?php echo $incident_verified_class;
					if ($current_page == 1 AND $i == 0) echo ' highlight'; ?>">
					<?php if ($incident_thumb) : ?>
					<div class="r_media">
					...
					<div class="r_details">
						<p class="r_location"><i class="icon-map-marker"></i> <a href="<?php echo $incident_url; ?>">Map</a></p>
